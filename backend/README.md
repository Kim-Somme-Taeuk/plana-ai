...
- `GET /ranking-snapshots/{snapshot_id}/validation-report`는 collector가 snapshot `note`에 남긴 구조화 가능한 진단 요약도 `collector_diagnostics`로 함께 반환합니다.
  - `collector_json`이 있으면 page별 summary, OCR stop hint, OCR stop recommendation까지 포함됩니다.
  - same payload에 `overlay_ignored_line_count`, `header_ignored_line_count`, `malformed_entry_line_count` 같은 파생 ignored OCR count도 포함됩니다.
  - same payload에 `empty_page_count`, `sparse_page_count`, `overlapping_page_count`, `stale_page_count`, `noisy_page_count` 같은 page quality signal count도 포함됩니다.
- `GET /seasons/{season_id}/validation-overview`는 시즌 단위 snapshot 상태 분포, valid/invalid 비율, top issue, issue 집계를 함께 반환합니다.
  - optional query: `status`, `source_type`, `collector_filter=with_diagnostics|capture_stop|hard_ocr_stop`, `capture_stop_reason`, `ocr_stop_reason`, `ignored_reason`, `ignored_group=overlay|header|malformed`, `page_signal=empty|sparse|overlapping|stale|noisy`, `ocr_stop_level=soft|hard`
- `GET /seasons/{season_id}/validation-overview`는 collector 진단이 남아 있는 snapshot 수, capture stop 수, hard OCR stop 수, ignored OCR line 총합도 함께 반환합니다.
- 같은 overview에는 `overlay_ignored_line_count`, `header_ignored_line_count`, `malformed_entry_line_count`도 함께 집계됩니다.
- 같은 overview에는 `empty_page_count`, `sparse_page_count`, `overlapping_page_count`, `stale_page_count`, `noisy_page_count`도 함께 집계됩니다.
- 같은 응답에 `capture_stop_reasons`, `ocr_stop_reasons`, `ignored_reasons` 집계도 포함됩니다.
- `GET /seasons/{season_id}/validation-series`는 snapshot별 invalid 비율과 top issue를 시간순으로 반환합니다.
  - optional query: `status`, `source_type`, `collector_filter=with_diagnostics|capture_stop|hard_ocr_stop`, `capture_stop_reason`, `ocr_stop_reason`, `ignored_reason`, `ignored_group=overlay|header|malformed`, `page_signal=empty|sparse|overlapping|stale|noisy`, `ocr_stop_level=soft|hard`
- `GET /seasons/{season_id}/validation-series`의 각 point에는 해당 snapshot의 `collector_diagnostics`가 포함될 수 있습니다.
- `GET /seasons/{season_id}/cutoff-series`는 completed snapshot만 사용하며 optional query `source_type`으로 source를 좁힐 수 있습니다.
