...
- `GET /ranking-snapshots/{snapshot_id}/validation-report`는 invalid 제외 수, duplicate rank 수, rank order violation 여부를 함께 반환합니다.
- `GET /seasons/{season_id}/validation-overview`는 시즌 단위 snapshot 상태 분포와 valid/invalid/issue 집계를 함께 반환합니다.
