...
- `player_name`이 `「Plana」`, `"Arona"`처럼 감싸져 있으면 바깥 감싸기 문자는 제거합니다.
...
- `내 순위`, `현재 순위`, `My Rank`, `Best Score` 같은 상태 표시 줄은 `status_line` reason으로 집계합니다.
...
- 마지막 페이지가 reward/ui/status overlay에 덮인 상태로 보이면 `overlay_last_page` hard 종료 힌트로 표시합니다.
- 마지막 페이지가 header/pagination 잡음 위주로 반복되면 `header_repeat_last_page` hard 종료 힌트로 표시합니다.
- 마지막 페이지에서 malformed OCR entry가 entry 수 이상으로 누적되면 `malformed_last_page` hard 종료 힌트로 표시합니다.
